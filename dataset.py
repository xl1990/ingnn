...
from syndataset import CustomDataset
...
class NCDataset(object):
    ...
    def get_idx_split(self, split_type='random', train_prop=.5, valid_prop=.25, seed=42):
        """
...
def load_nc_dataset(dataname, sub_dataname='', h=0.0, r=1):
    ...
    if dataname == 'twitch-e':
        ...
        dataset = load_coauthor_dataset(dataname)
    elif dataname in ('Computers', 'Photo'):
        dataset = load_amazon_dataset(dataname)
    elif dataname in ('syn-cora', 'syn-products'):
        dataset = load_syn_dataset(dataname, h, r)
    else:
...
# datasets used in DAGNN paper, split by choosing 20 samples each class as the training set
def load_planetoid_dataset(name):
    torch_dataset = Planetoid(root=f'{DATAPATH}/Planetoid',
                              name=name)
    data = torch_dataset[0]
    ...
    # the function to rewrite get_idx_split() in NCDataset
    def random_planetoid_splits(**kwargs):
        indices = []
        num_classes = dataset.label.max()+1
        for i in range(num_classes):
            index = (dataset.label.squeeze() == i).nonzero().view(-1)
        ...
        split_idx = {'train': train_index, 
                     'valid': rest_index[:500], 
                     'test': rest_index[500:1500]}

        return split_idx

    dataset.get_idx_split = random_planetoid_splits
    dataset.label = label
...
def load_coauthor_dataset(name):
    path = os.path.join(f'{DATAPATH}/Coauthor', name)
    torch_dataset = Coauthor(path, name)
    ...
    def random_coauthor_splits(**kwargs):
        indices = []
        num_classes = dataset.label.max()+1
        for i in range(num_classes):
            index = (dataset.label.squeeze() == i).nonzero().view(-1)
        ...
        # 20 samples each class as the training set
        # 30 samples each class as the validation set
        train_index = torch.cat([i[:20] for i in indices], dim=0)
    ...
    dataset.get_idx_split = random_coauthor_splits
    dataset.label = label
...
def load_amazon_dataset(name):
    path = os.path.join(f'{DATAPATH}/Amazon', name)
    torch_dataset = Amazon(path, name)
    ...
    def random_amazon_splits(**kwargs):
        indices = []
        num_classes = dataset.label.max()+1
        for i in range(num_classes):
            index = (dataset.label.squeeze() == i).nonzero().view(-1)
    ...
    dataset.get_idx_split = random_amazon_splits
    dataset.label = label

    return dataset


def load_syn_dataset(name, h, r):
    assert h in [i/10 for i in range(11)]
    assert r in [i+1 for i in range(3)]
    
    
    folder = "data/"+name
    filename = f"h{h:.2f}-r{r:1d}"
    syn_dataset = CustomDataset(root=folder, name=filename, 
                            setting="gcn", seed=15,require_mask=True)
    A = syn_dataset.adj
    features = syn_dataset.features
    label = syn_dataset.labels
    
    train_index = np.where(syn_dataset.train_mask)[0]
    valid_index = np.where(syn_dataset.val_mask)[0]
    test_index = np.where(syn_dataset.test_mask)[0]
...
#     print(np.sum(syn_dataset.train_mask))
#     print(np.sum(syn_dataset.val_mask))
#     print(np.sum(syn_dataset.test_mask))
#     def random_syn_splits(**kwargs):
#         split_idx = {'train': torch.tensor(train_index), 
#                      'valid': torch.tensor(valid_index), 
#                      'test': torch.tensor(test_index)}

#         return split_idx
    
    
    print("25-25-50 split")
    def random_syn_splits(**kwargs):
        train_idx, valid_idx, test_idx = rand_train_test_idx(dataset.label, train_prop=0.25, valid_prop=0.25, ignore_negative=True)
        split_idx = {'train': train_idx,
                     'valid': valid_idx,
                     'test': test_idx}
        return split_idx
    
    dataset.get_idx_split = random_syn_splits
